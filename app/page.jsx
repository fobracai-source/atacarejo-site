...
export default function HomePage() {
  const [atacarejo_products, setProducts] = useState([]);
  ...
  const categories = useMemo(
    () => [...new Set(atacarejo_products.map((p) => p.category).filter(Boolean))],
    [atacarejo_products]
  );
  const filtered = atacarejo_products
      ...
      <section className="bg-ink px-4 pb-8 pt-2 md:pb-10">
        <div className="mx-auto max-w-6xl">
          <h1 className="font-display text-2xl font-extrabold leading-tight text-white md:text-3xl">
            Preço baixo <span className="text-brand">todo dia</span>, em tudo 🛒
          </h1>
          <p className="mt-1.5 max-w-md text-sm text-white/70">
            Mercearia, limpeza, bebidas, eletrônicos e muito mais — direto do atacado pro seu carrinho, com entrega rápida.
          </p>
        </div>
      </section>
      <div className="mx-auto max-w-6xl">
