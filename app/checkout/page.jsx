...
export default function CheckoutPage() {
  ...
  useEffect(() => {
    async function loadDeliveryPaymentSettings() {
      const [moduleResult, optionsResult] = await Promise.all([
        supabase.from("atacarejo_module_settings").select("enabled").eq("id", "forma_pagamento_entrega").single(),
        supabase.from("atacarejo_delivery_payment_options").select("*").eq("enabled", true).order("sort_order"),
      ]);
  ...
  async function handleApplyCoupon() {
    ...
    const { data, error } = await supabase
      .rpc("atacarejo_check_coupon", { p_code: couponInput.trim(), p_order_total: subtotal })
      .single();
  ...
  async function handleConfirm(e) {
    ...
    try {
      ...
      if (customerId) {
        await supabase.from("atacarejo_customers").update(customerPayload).eq("id", customerId);
      } else {
        const { data: existing } = await supabase
          .from("atacarejo_customers").select("id").eq("phone", phone.trim()).maybeSingle();
        if (existing) {
          customerId = existing.id;
          await supabase.from("atacarejo_customers").update(customerPayload).eq("id", customerId);
        } else {
          const { data: newCustomer, error: customerError } = await supabase
            .from("atacarejo_customers").insert({ ...customerPayload, source: "site" }).select("id").single();
      ...
      const { data: order, error: orderError } = await supabase
        ...
        .insert({
          ...
          atacarejo_delivery_payment_options: mostrarComoVaiPagar ? selectedDeliveryPayments : null,
      ...
      const { error: itemsError } = await supabase.from("atacarejo_order_items").insert(orderItems);
      if (itemsError) throw itemsError;

      for (const i of items) {
        await supabase.rpc("atacarejo_decrement_stock", { p_product_id: i.product.id, p_quantity: i.quantity });
      }

      if (appliedCoupon) {
        await supabase.rpc("atacarejo_increment_coupon_usage", { p_code: appliedCoupon.code });
      }
